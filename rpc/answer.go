...
func (a *answer) fulfill(obj capnp.Ptr) error {
	...
	if err := a.conn.startWork(); err != nil {
		...
		retmsg := a.conn.newReturnMessage(a.id)
		ret, _ := retmsg.Return()
...
func (a *answer) reject(err error) error {
	...
	m := a.conn.newReturnMessage(a.id)
	mret, _ := m.Return()
...
func (qc *queueClient) handle(c *qcall) {
	...
	case qcallDisembargo:
		msg := qc.conn.newDisembargoMessage(
			rpccapnp.Disembargo_context_Which_receiverLoopback,
			c.embargoID)
		d, _ := msg.Disembargo()
...
func (qc *queueClient) rejectQueue() error {
	...
	for ; qc.q.Len() > 0; qc.q.Pop() {
		...
		case qcallDisembargo:
			m := qc.conn.newDisembargoMessage(
				rpccapnp.Disembargo_context_Which_receiverLoopback,
				c.embargoID)
			d, _ := m.Disembargo()
			d.SetTarget(c.embargoTarget)
